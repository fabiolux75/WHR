...
namespace LoocERP.Controllers.API {
    ...
    [ApiController]
    public class DdtController : ControllerBase
    {
...

        {
             ...
             foreach (var ddtItem in ddtlist)
             {
                ...
                string identificativo = ddtItem.Mittente.FiscalCode ?? ddtItem.Mittente.PIva.ToString();
                 ...
                 foreach (var dett in ddtItem.Dettagli)
                 {
                    ...
                    string descr_documento = "";
                    switch (ddtItem.Suffisso)
                    {
                        case "S":
                            descr_documento = "DDT di scarico";
                            break;
                        case "T":
                            descr_documento = "DDT di trasferimento";
                            break;
                        default:
                            descr_documento = "DDT";
                            break;
                    ...
                    string infoIntestazione = "false;" + ddtItem.NumeroProgressivoDocumento.ToString() + ";"
                                            ...
                                            + identificativo + ";;"+ descr_documento + ";";

                    if (dett.FlagAttrezzatura == 1) continue;  // Scartare dettagli con flag = 0
                    ...
                    csv += infoIntestazione 
                             ...
                             + dett.Codice + ";;;"
                             + dett.Descrizione + ";"
                             + TipoArticolo + ";" // Tipo Articolo (B o S)
                             + dett.UM + ";"
                             + codIvaDatev  + ";"
                             + aliquota_iva + ";"
                             + dett.Quantita + ";"
            ...
            try
            {
                ...
                string query = "UPDATE C_DDT SET DataExpDatev = getdate() WHERE Id IN  (@list)";
